    ...
    def readOrderData(self):
        ...
        for i in range(1,67):
            for timeFrag,orderNum in self.destPosTimefragDic[str(i)].items():
                tSet.add(timeFrag+'-' + str(i))
                tCount += orderNum
        print '订单终点在66个区域之外的条数为(成交了的情况)：',outZone
        print '总的订单数目为：',lineCount,'   成交了的订单数目为：',notNullCount
        print 'self.destPosTimeFragDic 中的时间片个数为：',len(tSet),'  订单条数为：',tCount
        print '总的时间片数目为：',len(timeFragSet)
        print tSet
...
class AssembleDics():
    ...
    def getThreeFragsOrders(self):
            ...
            for timeFrag,orderNum in self.destPosTimefragDic[str(i)].items():
                ...
                try:
                    try:
                        ...
                        #print str(i),last1Timefrag,'*************1**********'
                        fragSet.add(last1Timefrag+str(i))
                        last1 = 1
                    try:
                        last2 = self.destPosTimefragDic[str(i)][last2Timefrag]
                        print 'last2:',last2
                    except:
                        #print str(i),last2Timefrag,'**************2*********'
                        fragSet.add(last2Timefrag+str(i))
                        last2 = 2
                    try:
                        last3 = self.destPosTimefragDic[str(i)][last3Timefrag]
                        print 'last3:',last3
                    except:
                        #print str(i),last3Timefrag,'************3***********'
                        fragSet.add(last3Timefrag+str(i))
                        last3 = 3
                    #print '-------------------->',last1,last2,last3
                    destPosThreeDic[str(i)][timeFrag] =  last1 + last2 + last3
                    #print 'timefrap:----->>>>&&&&&:',timeFrag
        ...
        print 'length:',len(fragSet),'  count:',count
        #print destPosThreeDic
    ...
    def assemble(self):
        ...
        for predictDate in self.predict_data:
            for pos_field in range(1,67):
                for time_field in self.predict_time_fields:
                    ...
                    li = [[pos_field,noChangeTimeFrag],trafficList,weatherList,requestList,responseList,gapList,orderMeanPriceList,totalPriceList,poiList,[week],[isHoliday,frag,isRushHour],transOrdersList,[isWeek],[isWeatherDecrease],[sameZoneRate],[before3DestOrders]]
                    tempLi = []
                    #压平嵌套列表
        ...
        testDF = pd.DataFrame(self.rowsTestLi,columns=['pos','timeFrag','traffic1_1','traffic1_2','traffic1_3','traffic1_4','traffic2_1','traffic2_2','traffic2_3','traffic2_4',\
